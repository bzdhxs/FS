"""SCSD-HHO 消融实验：HHO → +SCI → +SCI+SDGE 三组，每组 30 次。

用法：
  python script/run_ssahho_ablation.py
  python script/run_ssahho_ablation.py --runs 3 --epoch 10 --pop 10   # 冒烟测试
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="SSAHHO 消融实验")
...
def main():
    ...
    out_dir   = PROJECT_ROOT / "log" / f"ablation_ssahho_{timestamp}"
    ...
    print(f"  Epoch={epoch}  Pop={pop}  split_seed={SPLIT_SEED}")
    ...
    for v_idx, variant in enumerate(VARIANTS):
        ...
        print(f"[{v_idx+1}/{len(VARIANTS)}] {label:16s}", end="", flush=True)
...
def _print_summary(summary_csv: Path, freq_csv: Path, variants: list) -> None:
    ...
    print("  消融实验汇总")
    print(f"{'='*80}")
    print(f"{'变体':<18} {'Test R²(mean±std)':<22} {'RMSE(mean±std)':<20} {'n_feat(mean±std)':<18} {'失败'}")
    print("-" * 80)
    for _, row in df_s.iterrows():
        r2_str    = f"{row['Test_R2_mean']}±{row['Test_R2_std']}"
        rmse_str  = f"{row['Test_RMSE_mean']}±{row['Test_RMSE_std']}"
        nfeat_str = f"{row['n_feat_mean']}±{row['n_feat_std']}"
        print(f"{row['variant']:<18} {r2_str:<22} {rmse_str:<20} {nfeat_str:<18} {row['fail_count']}")
